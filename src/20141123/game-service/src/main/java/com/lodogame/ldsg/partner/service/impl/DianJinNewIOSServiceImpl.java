...
import com.lodogame.game.dao.impl.mysql.PaymentOrderDaoMysqlImpl;
...
public class DianJinNewIOSServiceImpl extends BasePartnerService {

	public static Logger logger = Logger.getLogger(DianJinNewIOSServiceImpl.class);
    @Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if(DianJinNewIOSSdk.instance().verifySession(paras[0],paras[1])){
				return GameApiSdk.getInstance().loadUserToken(paras[1], partnerId, serverId,"0", params);
			}
		} catch (Exception e) {
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		DianJinPaymentObj_new cb = (DianJinPaymentObj_new) paymentObj;
		if(!DianJinNewIOSSdk.instance().checkPayCallbackSign(cb)) {
		...
		if (!"1".equals(cb.getPayStatus())) {
			...
			this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_ERROR,cb.getGoodsId(), finishAmount, "");
			return false;
		}
		int gold = (int) (finishAmount.doubleValue() * 10);
		// 更新订单状态
		if (this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_FINISH, cb.getGoodsId(), finishAmount, "")) {
			...
			if (!GameApiSdk.getInstance().doPayment(order.getPartnerId(), order.getServerId(), order.getPartnerUserId(), "", order.getOrderId(), finishAmount, gold, "", "")) {
				// 如果失败，要把订单置为未支付
				this.paymentOrderDao.updateStatus(order.getOrderId(), OrderStatus.STATUS_NOT_PAY, cb.getGoodsId(), finishAmount, "");
...
//	@Override
//	public String createOrder(String partnerId, String serverId, String partnerUserId, BigDecimal amount, String tradeName, String qn, String pkgId, String qn) {
