...
import com.easou.sso.sdk.util.ServerBuildUrlIOS;
...
public class EasouIOSServiceImpl extends BasePartnerService {

	public static Logger LOG = Logger.getLogger(EasouIOSServiceImpl.class);
...
/*
	...
	public String createOrder(String partnerId, String serverId, String partnerUserId, BigDecimal amount, String tradeName, String pkgId, String qn) {
		//if (StringUtils.isBlank(qn)) {
		   qn = "9000001";
		  // qn = ClientConfig.getProperty("qn");
		//}
		return this.createOrder(null, partnerId, serverId, partnerUserId, amount, tradeName, qn);
	}
	...
	public String createOrder(String orderId, String partnerId, String serverId, String partnerUserId, BigDecimal amount, String tradeName, String qn) {
		...
		tradeInfo.setNotifyUrl(EasouIOSSdk.instance().getPayBackUrl());
		tradeInfo.setAppId(EasouIOSSdk.instance().getAppId());
		tradeInfo.setPartnerId(EasouIOSSdk.instance().getEasouPartnerId());
		tradeInfo.setQn(qn);
		String url = ServerBuildUrlIOS.buildPayUrl(tradeInfo);
	...
	public boolean doPayment(String orderId, String partnerUserId, boolean success, String partnerOrderId, BigDecimal finishAmount, Map<String, String> reqInfo) {
		...
		if (paymentOrder.getStatus() == OrderStatus.STATUS_FINISH) {
			LOG.error("订单已经完成" + Json.toJson(paymentOrder));
			return true;
		}
		String extInfo = "";
		if (reqInfo != null) {
		...
		if (success) {// 成功
			...
			if (this.paymentOrderDao.updateStatus(orderId, OrderStatus.STATUS_FINISH, partnerOrderId, finishAmount, extInfo)) {
				...
				// 请求游戏服，发放游戏货币
				if (!GameApiSdk.getInstance().doPayment(paymentOrder.getPartnerId(), paymentOrder.getServerId(), paymentOrder.getPartnerUserId(), "", paymentOrder.getOrderId(), finishAmount, gold, "", "")) {
					LOG.info("发放游戏币失败:"+Json.toJson(paymentOrder));
					// 如果失败，要把订单置为未支付
