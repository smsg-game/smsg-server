...
public class DianJinNewServiceImpl extends BasePartnerService {

	public static Logger logger = Logger.getLogger(DianJinNewServiceImpl.class);
    @Autowired
	...
	public UserToken login(String token, String partnerId, String serverId, long timestamp, String sign, Map<String, String> params) {
		...
		try {
			...
			if(DianJinNewSdk.instance().verifySession(paras[1],paras[0])){
				String data="";
				if(StringUtils.isNotBlank(paras[2])){
					 data=paras[2];
				}else{
					data=paras[0];
				}
	...
	public boolean doPayment(PaymentObj paymentObj) {
		...
		if(!DianJinNewSdk.instance().checkPayCallbackSign(cb)) {
