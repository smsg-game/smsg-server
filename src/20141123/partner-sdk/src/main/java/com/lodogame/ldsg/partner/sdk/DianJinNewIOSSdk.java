...
public class DianJinNewIOSSdk extends BaseSdk {
	
	private static final Logger logger = Logger.getLogger(DianJinNewIOSSdk.class);
	
	private static DianJinNewIOSSdk ins;
	...
	public static DianJinNewIOSSdk instance() {
		synchronized (DianJinNewIOSSdk.class) {
			if (ins == null) {
				ins = new DianJinNewIOSSdk();
			}
		}
	...
	private void loadSdkProperties() {
		try {
			...
			appKey = prop.getProperty("DianJinNewIOSSdk.appKey");
			appId = prop.getProperty("DianJinNewIOSSdk.appId");
			partnerId = prop.getProperty("DianJinNewIOSSdk.partnerId");
			verifyUrl = prop.getProperty("DianJinNewIOSSdk.verifyUrl");
		} catch (IOException e) {
	...
	public boolean verifySession(String token,String uid){
		...
		if(StringUtils.isNotBlank(jsonStr)){
			...
			logger.info("点金IOS登录验证返回值："+jsonObject.getString("ErrorCode"));
